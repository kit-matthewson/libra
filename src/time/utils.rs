...
/// Gets the date of easter monday for a given year.
pub fn easter_monday(year: i32, orthodox: bool) -> Date {
    assert!(year >= MIN_DATE.year() && year <= MAX_DATE.year(), "year out of range");

    let day = EASTER_MONDAYS[usize::from(orthodox)][(year - MIN_DATE.year()) as usize];

    Date::from_ordinal_date(year, day).expect("could not construct easter monday date")
}

pub const MIN_DATE: Date = date!(1901-1-1);
pub const MAX_DATE: Date = date!(2199-12-31);

const EASTER_MONDAYS: [[u16; 299]; 2] = [
